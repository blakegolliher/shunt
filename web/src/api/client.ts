...
export interface PlacementStatus {
  ...
  legs?: { id: string; cluster: string; bucket: string; share: number }[] // a bucket spread over legs; primary and names are empty then
  ratio?: number
  prefixes?: string[]
...
export const clearTarget = (token: string, tenant: string, bucket: string) => request<{ key: string; target: string; name: string; version: number }>(`${apiRoot}/placements/${encodeURIComponent(tenant)}/${encodeURIComponent(bucket)}/target`, token, { method: 'DELETE' })
