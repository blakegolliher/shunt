...
export interface OwnedLeg { id: string; cluster: string; bucket: string; share: number; ranges: HashRange[] }
...
export function OwnershipBar({ legs, move, compact = false }: { legs: OwnedLeg[]; move?: MovingPart | null; compact?: boolean }) {
  ...
  return <div>
    ...
    {!compact && <ul className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted">{legs.map((l, i) => <li key={l.id} className="flex items-center gap-1.5"><span className={`inline-block h-2.5 w-2.5 rounded-full ${legColor(i)}`} /><span className="font-mono text-paper">{l.cluster}/{l.bucket}</span> {l.ranges.length ? pct(l.share) : 'idle'}{move?.from === l.id ? ' · moving out' : move?.to === l.id ? ' · moving in' : ''}</li>)}{move && <li className="flex items-center gap-1.5"><span className="inline-block h-2.5 w-2.5 rounded-full border border-paper bg-[repeating-linear-gradient(45deg,rgba(255,255,255,.7)_0,rgba(255,255,255,.7)_2px,transparent_2px,transparent_4px)]" />moving: {pct(move.share)} of keys</li>}</ul>}
  </div>
}
