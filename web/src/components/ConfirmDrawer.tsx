...
export function ConfirmDrawer({ open, title, summary, destructive = false, busy = false, onClose, onConfirm, children }:
  PropsWithChildren<{ open: boolean; title: string; summary: ReactNode; destructive?: boolean; busy?: boolean; onClose: () => void; onConfirm: () => void }>) {
  if (!open) return null
  return <div className="fixed inset-0 z-40 flex justify-end bg-black/70" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}>
    <section role="dialog" aria-modal="true" aria-labelledby="confirm-title" className="h-full w-full max-w-lg overflow-y-auto border-l border-ink-700 bg-ink-900 p-6 shadow-2xl">
      ...
      <div className="mt-8 flex justify-end gap-3">
        ...
        <button type="button" disabled={busy} className={`rounded-lg px-4 py-2 text-sm font-semibold text-white disabled:opacity-50 ${destructive ? 'bg-red-600 hover:bg-red-500' : 'bg-ember-600 hover:bg-ember-500'}`} onClick={onConfirm}>{busy ? 'Working…' : 'Confirm'}</button>
      </div>
    </section>
  </div>
}
