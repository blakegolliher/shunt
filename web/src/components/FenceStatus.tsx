export function FenceStatus({ held, waitingOn, version }: { held: boolean; waitingOn: string[]; version: number }) {
  if (held || waitingOn.length > 0) {
    return <div role="status" className="rounded-lg border border-ember-500/50 bg-ember-500/10 p-3 text-sm text-paper">
      ...
      <span className="ml-2">waiting on {waitingOn.length ? waitingOn.join(', ') : 'the held step'}</span>
    </div>
  }
