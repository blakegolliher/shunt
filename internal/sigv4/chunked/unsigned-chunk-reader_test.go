...
func TestUnsignedChunkReaderContentLengthMismatchStopsAtDecodedLength(t *testing.T) {
	...
	requireAPIErrorCode(t, err, errContentLengthMismatch.Code)
}
...
func TestUnsignedChunkReaderDeclaredChunkLongerThanPayloadReturnsIncompleteBody(t *testing.T) {
	...
	requireAPIErrorCode(t, err, s3.Lookup(s3.IncompleteBody).Code)
}
...
func TestUnsignedChunkReaderInvalidChunkSize(t *testing.T) {
	...
	requireAPIErrorCode(t, err, s3.Lookup(s3.InvalidChunkSizeError).Code)
...
func requireAPIErrorCode(t *testing.T, err error, code s3.Code) {
	t.Helper()
	if err == nil {
	...
	if apiErr.Code != code {
		t.Fatalf("s3.Error code = %q, want %q", apiErr.Code, code)
	}
}
