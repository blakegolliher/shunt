...
Re-run to separate code from machine noise: the `poc-1` tag and the POC-2 tree benchmarked alternately, two rounds of five each, same machine state. `poc-1` averaged 1783 ns/op, POC-2 1903 ns/op (+6.7 %). `internal/admin` is byte-identical between the two (`git diff poc-1 -- internal/admin` is empty), bytes and allocations per op are identical (1011 B, 10 allocs), and nothing in the benchmark loop touches POC-2 code. The difference is attributed to binary layout from the larger dependency graph, not a code regression. It exceeds the 5 % rule on a 2 µs handler and is flagged for the phase owner's decision rather than waived.
