...
| `github.com/aws/aws-sdk-go-v2` (+ `config`, `credentials`, `service/s3`) | v1.47.0 / s3 v1.113.1 | Apache-2.0 | SigV4-signing S3 client for the differential and bench harnesses (`test/s3diff`, `test/bench/s3bench`), the bucket emptier (`test/s3clean`) and the shared mover engine (`internal/mover`), invoked by `shunt migrate run` or a control-node browser operation. The proxy request path still signs with `internal/sigv4`; the SDK is used only by an explicit background mover. |
| `github.com/aws/smithy-go` | v1.28.1 | Apache-2.0 | The shared mover engine reads an S3 error's code (`PreconditionFailed`, `NotFound`) instead of matching error strings. Already pulled in by aws-sdk-go-v2. |
...
| `go.etcd.io/etcd/api/v3` | v3.7.1 | Apache-2.0 | `shunt-control` only: the API types the client exposes (member lists, status, and the key-value records a transaction's else branch reads back, to say why an operation reservation failed). |
