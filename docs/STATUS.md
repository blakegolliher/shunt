...
- **Two data races in the proxy, found by `-race` on the new tests.** `progressReader.n` counts the client request body, which net/http's transport reads on its own goroutine, while the handler read the count after `RoundTrip` returned; and the `httptrace` callbacks wrote connect and first-byte timings that `finish()` read from the handler goroutine, which overlap when a request is aborted. Both predate POC-3 (the trace pattern is POC-1's, the counter POC-2's) and neither could corrupt a response, but both are now fixed: the counter is atomic and the timings sit behind a small mutex.
