...
- **`features.xml_rewrite`** (default true, the one flag that defaults on, see `internal/config/features.go`) turns it off; `shunt serve` then warns, naming the clusters whose `<Location>` will leak.
