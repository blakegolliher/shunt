...
**Secrets are never in the directory.** A cluster names its secret only as a `secret_ref` (`env:NAME` or `file:/path`), resolved by the process that uses it. For a cluster added live, `file:` is the practical form: an `env:` var must already be in `shunt serve`'s environment.
