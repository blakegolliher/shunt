...
| S3 error catalog (code → HTTP status → message, XML rendering) | **Lift** | versitygw `s3err` | Apache-2.0 | The whole package if it has no Fiber dependency; otherwise the table and the XML writer | Same |
...
| Mover (copy engine, checkpointing, metadata/ACL/tag sync, rate limits, diff check) | **Run unmodified** | Chorus worker (`service/worker/copy`, `pkg/replication`, `pkg/tasks`) | Apache-2.0 | Run the worker binary as the POC and P5 mover against shunt-fronted buckets. Do not copy it in: it needs Redis/asynq and uses rclone (MIT), which conflicts with shunt's no-Redis, no-per-object-state rules. Its diff check is the CUTOVER verification | Running a tool imposes nothing; document the version used |
| Mover contract items Chorus lacks (`If-None-Match: *`, re-HEAD after copy, part-layout preservation, ratio-1 gate) | **Verify or wrap** | — | — | Check whether the worker's copy path does conditional PUT and preserves part layout. If not, test/mover in P5 stays the reference mover and Chorus is a bulk-copy option, not the contract | — |
...
- **POC-1 / P1:** `s3err` and `s3response` arrive on day one; error bodies and XML shapes are correct from the first s3diff run.
- **POC-4 / P5:** the mover question splits: Chorus worker for bulk copy if it honors the contract; `test/mover` as the contract reference either way.
