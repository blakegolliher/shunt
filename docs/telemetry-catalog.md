...
| `shunt_migration_dual_delete_total` | counter | `bucket`, `outcome` ∈ {`both`, `source_missing`, `source_failed`, `primary_failed`, `source_closed`} | deletes | — | Deletes sent to both clusters during RAMPING, MIGRATING and (since POC-5) CUTOVER, source first, then primary (ADR-0004 race 1). `source_failed`: the source leg failed and the primary leg was still sent; the object may come back when the mover copies it. `primary_failed`: the primary leg failed, whatever the source leg did; the client got the error and the object may be gone from the source while still on the primary (docs/migrating.md). Alert on both. `source_closed`: the source leg was not sent because purge-source has closed the source (its barrier, ADR-0021 D2); the source is about to be deleted | P5 | POC-4 |
...
| `shunt_migration_refused_writes_total` | counter | `bucket`, `reason` ∈ {`hold`, `stale`, `barrier`, `placement-read-only`, `cluster-read-only`} | writes | — | Writes and deletes shunt answered with 503 + Retry-After (or 403, read-only with reject) instead of routing (ADR-0016, ADR-0021 D2). `hold`: the key is inside a ramp step that has not reached every proxy yet. `barrier`: the bucket, or its cluster, carries a drain barrier (a held step, a read-only change being drained), so every mutation of it pauses until the change is committed; a few seconds per step on a healthy fleet. `stale`: this proxy has lost its lease with the control node and will not route a write on a bucket that is moving. `placement-read-only`, `cluster-read-only`: the maintenance switch. Alert on sustained `stale` or `barrier` | P5 | POC-6 |
