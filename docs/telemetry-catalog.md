...
| `shunt_fleet_members` | gauge | `state` ∈ {`live`, `silent`} | proxies | — | Control node only: members that have registered, by whether their heartbeat is within the lease (ADR-0016). A `silent` member blocks the first step of any bucket's migration until it returns or is forgotten | P3c | POC-6 |
...
| `shunt_fleet_fence_wait_seconds` | histogram | — | seconds | 1ms … 60s, log-spaced, 16 buckets | Control node only: time from a fenced change being written to every live member reporting it installed, per fence round (ADR-0016) | P3c | POC-6 |
