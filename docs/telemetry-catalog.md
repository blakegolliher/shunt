...
| `shunt_migration_refused_writes_total` | counter | `bucket`, `reason` ∈ {`hold`, `stale`, `barrier`, `source_closed`, `placement-read-only`, `cluster-read-only`} | writes | — | Writes and deletes shunt answered with 503 + Retry-After (or 403, read-only with reject) instead of routing (ADR-0016, ADR-0021 D2). `hold`: the key is inside a ramp step that has not reached every proxy yet. `barrier`: the bucket, or its cluster, carries a drain barrier (a held step, a read-only change being drained), so every mutation of it pauses until the change is committed; a few seconds per step on a healthy fleet. `source_closed`: a delete that must reach both clusters arrived while purge-source's source barrier is closed; it pauses for the purge rather than reach the primary alone (ADR-0021, the DELETE rule). `stale`: this proxy has lost its lease with the control node and will not route a write on a bucket that is moving. `placement-read-only`, `cluster-read-only`: the maintenance switch. Alert on sustained `stale` or `barrier` | P5 | POC-6 |
...
| `shunt_directory_cache_failures_total` | counter | `stage` ∈ {`write`, `fsync`, `rename`, `dir_fsync`} | failures | — | Members only: a restart-cache write that failed at a stage; the in-memory version stays and the cache is not durable for it; `/-/fleet` shows `durable` and `cache_error`, the heartbeat `durable` (ADR-0021 D1) | H1 | — |
| `shunt_runtime_bundles_retired` | gauge | — | bundles | — | Proxies in resign mode: replaced runtime bundles that a request still holds, each keeping its cluster set's transports open; at most 8 (ADR-0021 D1) | H1 | — |
| `shunt_install_backpressure` | gauge | — | — | — | Proxies in resign mode: 1 while an installed directory version waits for a retired runtime bundle to drain before new requests use it (8 are held); the heartbeat reports the version requests use, so a fence waits too. 0 otherwise (ADR-0021 D1) | H1 | — |
| `shunt_barrier_duration_seconds` | histogram | `phase` ∈ {`hold`, `drain`, `commit`, `settle`} | seconds | 10ms … 1h, log-spaced, 16 buckets | Control plane only: time a drain barrier spent in each phase (ADR-0021 D2). Implemented in H2d | H2 | — |
| `shunt_barrier_blockers` | gauge | `code` ∈ {`old_requests`, `backend_outcome_unknown`, `proxy_missing`, `install_pending`, `incarnation_unresolved`, `cache_not_durable`, `install_backpressure`, `multipart_open`, `worker_unresolved`, `source_diff`, `owner_lost`, `capability_missing`, `quorum_unavailable`} | blockers | — | Control plane only: blockers now holding open barriers, by code; alert on any held past the barrier's budget (ADR-0021 D2). Implemented in H2d | H2 | — |
| `shunt_lease_grant_errors_total` | counter | `reason` ∈ {`sequence`, `no_grant`, `behind`, `late`, `lineage`, `unreachable`} | heartbeats | — | Members only: a heartbeat whose answer granted no lease, by why (ADR-0021 D2, T07): the answer was to another heartbeat, carried no grant, named a version behind the member's or one it has not installed yet, arrived after the lease it grants had run out, came from another lineage, or never came. `/-/fleet` shows the last reason as `grant_error` | H2 | — |
| `shunt_fleet_unresolved_incarnations` | gauge | — | incarnations | — | Control plane only: proxy incarnations that ended without a clean retirement and still block barriers (ADR-0021 D2), summed over the fleet each time the control node reads it (at least once a second). Implemented in the H2 review fixes | H2 | — |
| `shunt_operations` | gauge | `status` ∈ {`pending`, `running`, `blocked`}, `effect` ∈ {`none`, `committed`, `uncertain`} | operations | — | Control plane only: unfinished operations, and ended ones whose effect is uncertain (`status` then the terminal one); alert on a blocked or uncertain one (ADR-0021 H0). Implemented in H2d | H2 | — |
