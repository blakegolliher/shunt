...
`cache_not_durable`, `install_backpressure`, `multipart_open`, `worker_unresolved`, `source_diff`,
...
| Register/run/resume mover (D2) | Existing mover operation gains durable generation-bound worker session; `POST /v1/operations/{id}/worker-heartbeat` carries session/sequence; status in operation record; `POST /v1/operations/{id}/resolve-worker` resolves an expired session with an attestation | Existing `shunt migrate run` enrolls before backend work; resume names existing operation; `shunt operation resolve-worker <id> --session <id> --attest <why>` | Existing browser mover shares server engine and session read model; the Operations drawer shows the worker session and asks for the attestation | T06, T09, T14, T17 |
