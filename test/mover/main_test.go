...
func moverFixture(t *testing.T, state string, ratio float64, targetConditional *bool) (*directory.File, *config.Config) {
	t.Helper()
	...
	if ratio > 0 {
		p.Ramp = &directory.Ramp{Ratio: ratio}
	}
	return &directory.File{Placements: map[string]directory.Placement{"acme/data": p}}, cfg
}
...
func TestMoverRefusesATargetWithoutConditionalPut(t *testing.T) {
	...
	for _, state := range []struct {
		...
		dir, cfg := moverFixture(t, state.state, state.ratio, &no)
		for _, sel := range [][2]string{{"acme/data", ""}, {"", "minio"}} {
			_, err := selectPlacements(dir, cfg, sel[0], sel[1], false)
			if err == nil {
			...
			jobs, err := selectPlacements(dir, cfg, sel[0], sel[1], true)
			if err != nil || len(jobs) != 1 || jobs[0].conditional {
				t.Errorf("%s: with -accept-lost-write-window: %v, %+v", state.name, err, jobs)
			}
		}
		dir, cfg = moverFixture(t, state.state, state.ratio, &yes)
		if jobs, err := selectPlacements(dir, cfg, "acme/data", "", false); err != nil || len(jobs) != 1 || !jobs[0].conditional {
