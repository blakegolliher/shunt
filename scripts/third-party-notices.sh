...

{
  ...
  echo
  echo "==================== PRODUCTION PACKAGES LINKED INTO shunt web (generated by make ui) ===================="
  cat web/THIRD_PARTY_NOTICES
} > "$tmp/new"
# Upstream license files sometimes carry CRLF or stray trailing spaces. Preserve their text while
# normalizing line endings so the generated repository artifact passes the whitespace gate.
sed -i 's/[[:space:]]\+$//' "$tmp/new"
mv "$tmp/new" "$out"
